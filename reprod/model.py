...
def create_models(input_shape: tuple, learning_rate: float,
    ...
    if 'GRU' in models:
        regressor = Sequential()
        regressor.add(GRU(units=3, activation='relu',
                          return_sequences=True, input_shape=input_shape))
        regressor.add(GRU(units=6, activation='relu'))
        regressor.add(Dense(units=horizons))
        regressor.summary()
        regressor.compile(optimizer='adam', loss='mean_squared_error')
        regressors.append(regressor)
    ...
    if 'LSTM' in models:
        regressor = Sequential()
        regressor.add(LSTM(units=3, activation='relu',
                           return_sequences=True, input_shape=input_shape))
        regressor.add(LSTM(units=6, activation='relu'))
        regressor.add(Dense(units=horizons))
        regressor.summary()
        regressor.compile(optimizer='adam', loss='mean_squared_error')
        regressors.append(regressor)
